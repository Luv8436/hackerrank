...
struct data{
    struct SinglyLinkedListNode *add;
    struct data *link;
};
...
bool has_cycle(SinglyLinkedListNode* head) {
    ...
    while(ptr!=NULL)
    {
        ...
        if(i==1)
        {
            start=end=(struct data *)malloc(sizeof(struct data));
            start->add=ptr;
            start->link=NULL;
        }
        else{
            avail=(struct data *)malloc(sizeof(struct data));
            avail->add=ptr;
            avail->link=NULL;
            end->link=avail;
            end=avail;
        }
        struct data *pointer=start;
        
        while(pointer!=NULL){
            if(pointer->add==ptr->next)
            return true;
